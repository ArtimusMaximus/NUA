...
export default function Navbar({ title, themeValue, callBackChanged })
{
  ...
  const [newTheme, setNewTheme] = useState({ theme: themeValue || 'dark' });
  ...
  const updateTheme = async () => {
    if (themeValue === 'light' || themeValue === '') {
      // console.log(`new theme before ${newTheme}`);
      setNewTheme({ theme: 'dark' });
      console.log('new theme after', newTheme);
    } else {
      setNewTheme({ theme: 'light' });
      console.log('new theme in else: ', newTheme);
    }
    try {
